...
def page_catalog(conn, user):
    ...
    with tabs[2]:
        ...
        df_out = fetch_df(conn, "SELECT code,name FROM products WHERE cat_code=:c ORDER BY name",
                          {"c": out_cat})
        ...
        # ——— Nguồn NVL cho MỨT (2 loại) ———
        src = None
        fruits_chosen, cots_chosen = [], []
        if typ=="MUT":
            src = st.radio("Nguồn NVL mứt", ["TRAI_CAY","COT"], horizontal=True, index=0, key="mut_src")
            if src=="TRAI_CAY":
                ...
                fruits_chosen = st.multiselect("Chọn trái cây (chỉ để hiển thị khi sản xuất)", fruit_opts)
            else:
                df_cot = fetch_df(conn, "SELECT code,name FROM products WHERE cat_code='COT' ORDER BY name")
                cot_opts = [f"{r['code']} — {r['name']}" for _,r in df_cot.iterrows()]
                cots_chosen = st.multiselect("Chọn CỐT (chỉ để hiển thị khi sản xuất)", cot_opts)
        else:
            st.caption("Nguồn NVL CỐT mặc định là trái cây + NVL khác theo định mức.")

        # ——— NVL khác cần định mức (kg / 1kg sơ chế) ———
        st.markdown("##### Nguyên liệu khác (kg / 1kg SƠ CHẾ)")
        df_other = fetch_df(conn, """
            SELECT code,name FROM products
            WHERE cat_code IN ('PHU_GIA','TP_KHAC') ORDER BY name
        """)
        other_opts = [f"{r['code']} — {r['name']}" for _,r in df_other.iterrows()]
        picked_other = st.multiselect("Chọn NVL", other_opts)
        other_inputs = {}
        ...
        if st.button("💾 Lưu CT", type="primary"):
            if not code or not name or not output_pcode:
                ...
                note_dict = {}
                if typ=="MUT":
                    note_dict["src"] = src or "TRAI_CAY"
                    if fruits_chosen:
                        note_dict["fruits"] = [x.split(" — ",1)[0] for x in fruits_chosen]
                    if cots_chosen:
                        note_dict["cots"] = [x.split(" — ",1)[0] for x in cots_chosen]
                ...
                """, {"c": code.strip(), "n": name.strip(), "t": typ, "o": output_pcode,
                      "r": float(recovery), "k": float(cups_per_kg), "x": json.dumps(note_dict, ensure_ascii=False)})
